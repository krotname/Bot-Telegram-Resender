...
def start(message):
    bot.send_message(message.chat.id, 'Здравствуйте! \n\nДля заказа пропуска на автомобиль в подземный паркинг отправьте заявку в формате: \n\n🏢 Башня для разгрузки\n⏰ Дата и время въезда\n🚘 Марка автомобиля\n🆔 Гос. номер автомобиля\n\nДля помощи введите /help')


@bot.message_handler(commands=["help"])
def start(message):
    bot.send_message(message.chat.id, 'В случае возникновения проблем при заказе пропуска- обратитесь к администратору Вашей башни в холле первого этажа.👨')
...
def messages(message):
    ...
    while x < len(whitelist):
     if message.from_user.username == whitelist[x]:
        bot.send_message(group1, str(message.from_user.username) + ': ' + str(message.from_user.first_name) + ': ' + str(message.from_user.last_name) + ': ' + message.text)
